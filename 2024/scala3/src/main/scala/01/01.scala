...
def makeLists(file: String) =
  val maybeNums = file
    ...
    .map: line =>
      ...
      List(
        spaceSplit.headOption.flatMap(_.toIntOption),
        spaceSplit.lastOption.flatMap(_.toIntOption)
      )
  (maybeNums.map(_.head).flatten, maybeNums.map(_.last).flatten)

def part1(list1: Array[Int], list2: Array[Int]) =
  list1.sorted
    .zip(list2.sorted)
    .map({ case (first, second) => (first - second).abs })
    .sum
